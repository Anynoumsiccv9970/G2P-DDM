...
class PoseVQVAE(pl.LightningModule):
    def __init__(self, args, text_dict):
        ...
        # vqvae
        self.tem_pos_emb = nn.Parameter(torch.zeros(2000, args.embedding_dim))
        self.spa_pos_emb = nn.Parameter(torch.zeros(3, args.embedding_dim))
        ...
        self.enc_tem_vit = Encoder(dim=args.embedding_dim, depth=3, heads=8, mlp_dim=1024, dropout = 0.1)
        self.enc_spa_vit = Encoder(dim=args.embedding_dim, depth=3, heads=8, mlp_dim=1024, dropout = 0.1)
        
        self.codebook_pose = Codebook(n_codes=args.n_codes, embedding_dim=args.embedding_dim)
        self.codebook_rhand = Codebook(n_codes=args.n_codes, embedding_dim=args.embedding_dim)
        self.codebook_lhand = Codebook(n_codes=args.n_codes, embedding_dim=args.embedding_dim)
    ...
    def concat_codebook(self,):
        self.codebook = Codebook(n_codes=self.args.n_codes*3, embedding_dim=self.args.embedding_dim)
        self.codebook.embeddings = torch.cat([self.codebook_pose.embeddings, self.codebook_rhand.embeddings, self.codebook_lhand.embeddings], dim=0)
        self.codebook.embeddings.require_grads = False
    ...
    def vqvae_encode(self, points, points_mask):
        ...
        # print("points_feat: ", points_feat.shape)
        vq_output_pose = self.codebook_pose(points_feat[...,0:1])
        vq_output_rhand = self.codebook_rhand(points_feat[...,1:2])
        vq_output_lhand = self.codebook_lhand(points_feat[...,2:3])
        # vq_output = torch.cat([vq_output_pose, vq_output_rhand, vq_output_lhand], dim=-1)
        
        vq_tokens_pose, points_feat_pose, commitment_loss_pose = vq_output_pose['encodings'], vq_output_pose['embeddings'], vq_output_pose["commitment_loss"] # [bs, max_len, 3]
        vq_tokens_rhand, points_feat_rhand, commitment_loss_rhand = vq_output_rhand['encodings'], vq_output_rhand['embeddings'], vq_output_rhand["commitment_loss"] # [bs, max_len, 3]
        vq_tokens_lhand, points_feat_lhand, commitment_loss_lhand = vq_output_lhand['encodings'], vq_output_lhand['embeddings'], vq_output_lhand["commitment_loss"] # [bs, max_len, 3]
        
        # print(vq_tokens_pose.shape, vq_tokens_rhand.shape, vq_tokens_lhand.shape)
        # print(points_feat_pose.shape, points_feat_rhand.shape, points_feat_lhand.shape)

        vq_tokens = torch.cat([vq_tokens_pose, vq_tokens_rhand, vq_tokens_lhand], dim=-1) # [bs, t, n]
        vq_tokens_concat = torch.cat([vq_tokens_pose, vq_tokens_rhand + self.args.n_codes, vq_tokens_lhand + self.args.n_codes*2], dim=-1)
        points_feat = torch.cat([points_feat_pose, points_feat_rhand, points_feat_lhand], dim=-1)
        commitment_loss = commitment_loss_pose + commitment_loss_rhand + commitment_loss_lhand
        return vq_tokens, points_feat, commitment_loss, vq_tokens_concat
    ...
    def forward(self, batch, mode, vis_func, vis_tok_func):
        ...
        # vqvae encoder
        vq_tokens, points_feat, commitment_loss, _ = self.vqvae_encode(points, points_mask) # [bs, t], [bs, h, t]
    ...
    #     pred_img = self.points2imgs(dec_points, skel_len) * 255.
    #     ori_img = self.points2imgs(ori_points, skel_len) * 255.
    ...
    #         cv2.imwrite("/Dataset/everybody_sign_now_experiments/vqvae_recons/{}.png".format(name), show_img)
    ...
    def configure_optimizers(self):
        self.optimizer = torch.optim.Adam(self.parameters(), lr=self.args.init_lr, betas=(0.9, 0.999))
