...
class BackTranslateModel(pl.LightningModule):
    def __init__(self, args, text_dict):
        ...
        self.points_emb = nn.Linear(150, self.hidden_size)

        self.conv = nn.Sequential(nn.Conv1d(self.hidden_size, self.hidden_size, kernel_size=5, stride=1, padding=2), 
                                   nn.BatchNorm1d(self.hidden_size), 
                                   nn.LeakyReLU(),
                                   nn.MaxPool1d(2, 2),
                                   nn.Conv1d(self.hidden_size, self.hidden_size, kernel_size=5, stride=1, padding=2),
                                   nn.BatchNorm1d(self.hidden_size),
                                   nn.MaxPool1d(2, 2))

        self.ctc_out = nn.Linear(self.hidden_size, len(text_dict))
    ...
    def forward(self, points, skel_len, word_tokens, word_len, mode):
        ...
        # max_len = max_len // 4
        max_len = torch.div(max_len, 4, rounding_mode="floor")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i, length in enumerate(gloss_len):
            ref = gloss_id[i][:length].tolist()[:-1] # [:-1]
