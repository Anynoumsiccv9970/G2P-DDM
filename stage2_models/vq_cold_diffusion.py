...
class Point2textModelStage2(pl.LightningModule):
    def __init__(self,
        ...
        self.decoder = Decoder(dim=hidden_size, depth=depth, heads=heads, mlp_dim=hidden_size*4, dropout=dropout, diffusion_step=diffusion_step)
    ...
    # def q_posterior_cold(self, log_x_start, log_x_t, t):
    #     pass
    ...
    def forward(self, batch, mode):
        ...
        with torch.no_grad():
            ...
            # print("vq_tokens: ", vq_tokens.shape)
            # print("x_0: ", x_0.shape)
    ...
    def generate(self, batch, batch_idx):
        ...
        with torch.no_grad():
            if not self.cold_diffusion:
                ...
                # print("use cold diffusion")
                for diffusion_index in range(start_step-1, 0, -1):
                    t = torch.full((bs,), diffusion_index, device=device, dtype=torch.long)
                    log_x_recon = self.predict_start(log_z, src_feat, t, x_mask, src_mask)
                    log_z = log_z - self.q_sample(log_x_recon, t, x_mask) + self.q_sample(log_x_recon, t-1, x_mask)
